...
public class RUISLeftHandGesutre : RUISGestureRecognizer
{
	...
	{		
		...
		if (!gestureStarted) {
			...
			if (dist < distanceTreshold) {
				Debug.Log ("Lefty Gesture started");
				gestureStarted = true;
			}
		} 
		else if(!startCnt) {
			...
			{
				Debug.Log ("Lefty Start counting");
				startCnt = true;
				gestureStarted = false;
			}
		}
		...
		if (startCnt) 
		{
			...
			if(cnt2 == 30)
			{
				Debug.Log ("Lefty Throwing");
				cnt2 = 0;
				startCnt = false;
				Magic magic = MagicFactory.GetMagic("ZShape");
				magic.TryEffect();
			}
		}
